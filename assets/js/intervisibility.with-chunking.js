...
(function() {
    ...
    function showIntervisibilityFieldSelectionModal(geoJson) {
        ...
        if (pointFeatures.length === 0) {
            console.warn('showIntervisibilityFieldSelectionModal: No Point features found');
            updateIntervisibilityStatus('No Point features found in GeoJSON file.');
            return;
        }
        
        console.log('showIntervisibilityFieldSelectionModal: Found', pointFeatures.length, 'Point features');
    ...
    window.createIntervisibilityMatrix = async function() {
        ...
        if (!storageInit.success) {
            updateIntervisibilityStatus('Warning: Could not initialize chunked storage. Proceeding with in-memory calculation (may fail with large site counts).');
        } else {
            updateIntervisibilityStatus(`Using ${storageInit.type === 'fsa' ? 'File System Access API' : 'Downloads folder'} for chunked storage.`);
        }
        
        // Determine if we should use chunking
        // Enable chunking if site count > CHUNK_SIZE (30 sites)
        // Note: With File System Access API, chunks can be loaded back for pair checking
        // With Downloads folder, chunks are saved but cannot be loaded back - in this case,
        // we'll process all profiles, save them, then use them in memory for pair checking
        ...
        if (useChunking) {
            ...
            updateIntervisibilityStatus('Processing all sites in memory (small site count)...');
        }
        ...
        if (useChunking) {
            // CHUNKED PROCESSING: Process sites in chunks
            ...
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                ...
                // Calculate profiles for this chunk
                const chunkProfiles = [];
                for (let i = 0; i < chunkMarkers.length; i++) {
                    ...
                    try {
                        ...
                        // Store profile data for this chunk
                        chunkProfiles.push({
                ...
                // Save chunk profiles to temp file
                if (chunkProfiles.length > 0 && !cancelCalculation) {
                    updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Saving profiles to temp file...`);
                    const saveResult = await saveChunkData(chunkIdx, 'profiles', {
                        chunkIndex: chunkIdx,
                ...
                // Clear chunk profiles from memory
                chunkProfiles.length = 0;
                
                // Clean cache after each chunk
                cleanupDemCache();
                if (window.gc) {
                    try { window.gc(); } catch (e) {}
                }
            ...
            // NON-CHUNKED PROCESSING: Original logic for small site counts
        ...
        if (useChunking && canLoadChunks) {
            // CHUNKED PAIR CHECKING: Load profiles incrementally and check pairs (File System Access API)
            const totalChunks = Math.ceil(intervisibilityMarkers.length / CHUNK_SIZE);
            ...
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                ...
                updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Loading profiles and checking pairs...`);
                
                // Load current chunk profiles (only works with File System Access API)
                const loadResult = await loadChunkData(chunkIdx, 'profiles');
                ...
                for (let idx1 = 0; idx1 < currentChunkIndices.length; idx1++) {
                    ...
                    for (let idx2 = idx1 + 1; idx2 < currentChunkIndices.length; idx2++) {
                        ...
                        checked++;
                        updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Checking pair ${checked}/${total} (${marker1.name} ↔ ${marker2.name})...`);
                ...
                // Check pairs with previous chunks (if any)
                for (let prevChunkIdx = 0; prevChunkIdx < chunkIdx; prevChunkIdx++) {
                    if (cancelCalculation) break;
                    
                    updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Checking pairs with chunk ${prevChunkIdx + 1}...`);
                    
                    const prevLoadResult = await loadChunkData(prevChunkIdx, 'profiles');
                    if (!prevLoadResult.success) {
                        console.warn(`Could not load previous chunk ${prevChunkIdx} profiles for cross-chunk checking`);
                        continue;
                    }
                    
                    const prevChunkData = prevLoadResult.data;
                    const prevChunkProfiles = new Map();
                    prevChunkData.profiles.forEach(prof => {
                        prevChunkProfiles.set(prof.globalIndex, {
                            profile: prof.profile.map(p => ({
                                x: p.x,
                                y: p.y,
                                latlng: (p.lat !== null && p.lng !== null) ? L.latLng(p.lat, p.lng) : null
                    ...
                    // Check all pairs between current chunk and previous chunk
                    const prevChunkIndices = Array.from(prevChunkProfiles.keys()).sort((a, b) => a - b);
                    
                    for (const i of currentChunkIndices) {
                        if (cancelCalculation) break;
                        ...
                        for (const j of prevChunkIndices) {
                            if (cancelCalculation) break;
                            ...
                            checked++;
                            updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Cross-chunk pair ${checked}/${total} (${marker1.name} ↔ ${marker2.name})...`);
                ...
                // Save chunk pair results to temp file
                if (chunkVisiblePairs.length > 0 && !cancelCalculation) {
                    updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Saving pair results to temp file...`);
                    const saveResult = await saveChunkData(chunkIdx, 'pairs', {
                        chunkIndex: chunkIdx,
                        sessionId: chunkedStorageSessionId,
                        pairCount: chunkVisiblePairs.length,
                        pairs: chunkVisiblePairs.map(p => ({
                            marker1Idx: p.marker1Idx,
                            marker2Idx: p.marker2Idx,
                            marker1Lat: p.marker1.latlng.lat,
                            marker1Lng: p.marker1.latlng.lng,
                            marker2Lat: p.marker2.latlng.lat,
                            marker2Lng: p.marker2.latlng.lng
                        }))
                    });
                ...
                // Add to accumulated visible pairs (convert back to marker objects)
                chunkVisiblePairs.forEach(p => {
                    allVisiblePairs.push({
                        marker1: p.marker1,
                        marker2: p.marker2
                    });
                });
            ...
            // Merge all visible pairs from all chunks
            visiblePairs.push(...allVisiblePairs);
        } else if (useChunking && !canLoadChunks) {
            // CHUNKED PROCESSING WITH DOWNLOADS FOLDER: Cannot load chunks back
            // In this case, we need to recalculate profiles in memory for pair checking
            // This is a limitation of Downloads folder - files are saved but cannot be loaded back
            updateIntervisibilityStatus('Phase 2: Downloads folder used - recalculating profiles in memory for pair checking...');
            updateIntervisibilityStatus('Note: Profiles were saved to Downloads folder but cannot be loaded back programmatically.');
            
            // Recalculate all profiles in memory (chunked to manage memory)
            ...
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                ...
                updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Recalculating profiles for pair checking...`);
                ...
                for (let i = startIdx; i < endIdx; i++) {
                    ...
                    try {
                        ...
                        if (!cancelCalculation) {
                            console.error(`Error recalculating profile for pair checking: ${marker.name}:`, error);
                        }
                    }
            ...
            // Now check pairs using the in-memory profiles (same logic as non-chunked)
            ...
            // Clear markerDataForPairs from memory
            markerDataForPairs.clear();
        } else {
            // NON-CHUNKED PAIR CHECKING: Original logic
        ...
        // Cleanup temp files if using File System Access API (optional - user can keep files)
        if (useChunking && chunkedStorageType === 'fsa') {
            // Optionally clean up temp files - comment out if you want to keep them for debugging
