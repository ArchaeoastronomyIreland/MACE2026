...
(function() {
    ...
    async function calculateMarkerProfile(marker, map, siteNumber = null, totalSites = null) {
        ...
        // Prefix for site count if provided
        const sitePrefix = (siteNumber !== null && totalSites !== null) ? `Site ${siteNumber} of ${totalSites}: ` : '';
        ...
        const dem = await window.HC_fetchTerrainPatch(latlng, zoom, radiusTiles, map, (dl, tot) => {
            updateIntervisibilityStatus(`${sitePrefix}Calculating profile for ${marker.name}: Downloading tiles ${dl}/${tot}...`);
        });
        ...
        updateIntervisibilityStatus(`${sitePrefix}Calculating profile for ${marker.name}: Computing viewshed...`);
    ...
    // Show important warning message with warning icon
    function showIntervisibilityWarning(message) {
        const statusEl = document.getElementById('intervisibility-status');
        const statusTextEl = document.getElementById('intervisibility-status-text');
        if (statusEl && statusTextEl) {
            statusTextEl.textContent = message;
            ...
            const spinner = statusEl.querySelector('i');
            if (spinner) {
                spinner.classList.remove('fa-spinner', 'fa-spin', 'fa-info-circle');
                spinner.classList.add('fa-exclamation-triangle');
            }
            // Set warning styling
            ...
            console.warn('Intervisibility Warning:', message);
        }
    }
    ...
    function updateIntervisibilityStatistics(totalMarkers, totalPairs, intervisiblePairs, visibilityPercent, pairsChecked) {
        ...
        if (window.visiblePairsForStats && window.visiblePairsForStats.length > 0) {
            ...
            // Ensure Generate IV Report button is enabled whenever we have IV results
            $('#btn-generate-iv-report').prop('disabled', false);
        }
    }
    ...
    window.createIntervisibilityMatrix = async function() {
        ...
        // Determine if we should use chunking (only for large site counts)
        // CRITICAL: Only initialize chunked storage if we actually need it (> 30 sites)
        // This prevents unnecessary directory picker prompts for small site counts
        const CHUNK_SIZE = (typeof window.IV_CHUNK_SIZE !== 'undefined') ? window.IV_CHUNK_SIZE : 30;
        let useChunking = intervisibilityMarkers.length > CHUNK_SIZE && typeof window.IV_initializeChunkedStorage === 'function';
        let storageInit = null;
        let canLoadChunks = false;
        
        if (useChunking) {
            updateIntervisibilityStatus('Initializing chunked storage for temp files...');
            storageInit = await window.IV_initializeChunkedStorage();
            if (!storageInit || !storageInit.success) {
                // User cancelled or storage failed - warn and ask to continue
                const continueCalculation = confirm('⚠️ Warning: No temporary file storage available.\n\n' +
                    'Without temporary files, the calculation will run entirely in memory and may hang your browser with large site collections.\n\n' +
                    'Do you want to continue with in-memory calculation?');
                ...
                if (!continueCalculation) {
                    ...
                    isCalculating = false;
                    cancelCalculation = false;
                    updateIntervisibilityStatus('Calculation cancelled by user.');
                    return;
                }
                
                showIntervisibilityWarning('⚠️ Warning: Proceeding with in-memory calculation (no temporary file storage). Browser may hang with large site counts.');
                useChunking = false;
            } else {
                // Show storage confirmation message
                const dirName = storageInit.directoryName || (storageInit.type === 'indexeddb' ? 'Browser storage (IndexedDB)' : 'Downloads folder');
                const storageMsg = storageInit.type === 'indexeddb' 
                    ? `💾 Using browser storage (IndexedDB) for temporary files.\n\nData will be stored automatically - no user interaction required.`
                    : `📁 Temporary files directory: ${dirName}\n\nFiles will be saved to this location during calculation.`;
                showIntervisibilityWarning(storageMsg);
                canLoadChunks = storageInit.success && storageInit.canLoad === true;
                updateIntervisibilityStatus(`Using chunked processing (${CHUNK_SIZE} sites per chunk) to manage memory...`);
            }
        }
        ...
        const markerData = useChunking ? null : new Map(); // Only use Map if not chunking
        markerDems.clear();
        let profilesCompleted = 0;
        
        if (useChunking) {
            // CHUNKED PROCESSING: Process sites in chunks (see intervisibility-chunking.js for functions)
            const totalChunks = Math.ceil(intervisibilityMarkers.length / CHUNK_SIZE);
            
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                if (cancelCalculation) break;
                
                const startIdx = chunkIdx * CHUNK_SIZE;
                const endIdx = Math.min(startIdx + CHUNK_SIZE, intervisibilityMarkers.length);
                const chunkMarkers = intervisibilityMarkers.slice(startIdx, endIdx);
                
                updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Calculating profiles for sites ${startIdx + 1}-${endIdx}...`);
                ...
                for (let i = 0; i < chunkMarkers.length; i++) {
                    ...
                    const marker = chunkMarkers[i];
                    updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Calculating profile ${i + 1}/${chunkMarkers.length} (Site ${globalIdx + 1}/${intervisibilityMarkers.length}): ${marker.name}...`);
                    
                    try {
                        const result = await calculateMarkerProfile(marker, map, globalIdx + 1, intervisibilityMarkers.length);
                ...
                if (chunkProfiles.length > 0 && !cancelCalculation && typeof window.IV_saveChunkData === 'function') {
                    updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Saving profiles to temp file...`);
                    const saveResult = await window.IV_saveChunkData(chunkIdx, 'profiles', {
                        chunkIndex: chunkIdx,
                        siteCount: chunkProfiles.length,
                        profiles: chunkProfiles
                    });
                    
                    if (!saveResult.success) {
                        console.error(`Failed to save chunk ${chunkIdx} profiles:`, saveResult.error);
                        // If first chunk fails and no storage available, warn user
                        if (chunkIdx === 0) {
                            const continueCalc = confirm('⚠️ Warning: Failed to save temporary files.\n\n' +
                                `Error: ${saveResult.error || 'No storage method available'}\n\n` +
                                'The calculation will continue in memory, which may cause the browser to hang.\n\n' +
                            ...
                            showIntervisibilityWarning('⚠️ Warning: Temporary file saving failed. Calculation continuing in memory - browser may hang.');
                        } else {
                            updateIntervisibilityStatus(`Warning: Failed to save chunk ${chunkIdx} profiles. Error: ${saveResult.error || 'Unknown error'}`);
                        }
                    } else {
                        const storageType = saveResult.storageType || 'saved';
                        if (storageType === 'downloads') {
                            updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Profile file saved to Downloads folder (${saveResult.fileName})`);
                        } else {
                            updateIntervisibilityStatus(`Phase 1: Chunk ${chunkIdx + 1}/${totalChunks}: Profile file saved (${saveResult.fileName})`);
                        }
                    }
                }
            ...
            // NON-CHUNKED PROCESSING: Original logic for small site counts (≤30 sites)
            for (let i = 0; i < intervisibilityMarkers.length; i++) {
                ...
                try {
                    const result = await calculateMarkerProfile(marker, map, i + 1, intervisibilityMarkers.length);
                    // Store only profile and observerH - we don't need to keep the DEM in memory
        ...
        let checked = 0;
        const visiblePairs = []; // Store visible pairs to avoid duplicates (will be accumulated)
        
        updateIntervisibilityStatus(`Phase 2: Checking intervisibility between ${profilesCompleted} marker pairs...`);
        
        if (useChunking && canLoadChunks) {
            // CHUNKED PROCESSING WITH INDEXEDDB: Load profiles incrementally and check pairs
            const totalChunks = Math.ceil(intervisibilityMarkers.length / CHUNK_SIZE);
            const allVisiblePairs = []; // Accumulate all visible pairs from all chunks
            
            // We need to check all pairs across all chunks, so we'll load chunks incrementally
            // and compare with previously loaded chunks
            const loadedChunkProfiles = []; // Array of Maps, one per chunk
            
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                if (cancelCalculation) break;
                
                updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Loading profiles from IndexedDB...`);
                
                // Load current chunk profiles from IndexedDB
                const loadResult = await window.IV_loadChunkData(chunkIdx, 'profiles');
                if (!loadResult.success || !loadResult.data) {
                    console.error(`Failed to load chunk ${chunkIdx} profiles:`, loadResult.error);
                    updateIntervisibilityStatus(`Error: Could not load chunk ${chunkIdx} profiles. Cannot continue with pair checking.`);
                    break;
                }
                
                const chunkData = loadResult.data;
                const currentChunkProfiles = new Map();
                
                // Restore profiles to Map for easy access
                if (chunkData.profiles && Array.isArray(chunkData.profiles)) {
                    chunkData.profiles.forEach(prof => {
                        currentChunkProfiles.set(prof.globalIndex, {
                            profile: prof.profile.map(p => ({
                                x: p.x,
                                y: p.y,
                                latlng: (p.lat !== null && p.lng !== null) ? L.latLng(p.lat, p.lng) : null
                            })),
                            observerH: prof.observerH
                        });
                    });
                }
                
                loadedChunkProfiles.push(currentChunkProfiles);
                
                // Check pairs: current chunk vs all previously loaded chunks
                const currentChunkIndices = Array.from(currentChunkProfiles.keys()).sort((a, b) => a - b);
                
                for (let idx1 = 0; idx1 < currentChunkIndices.length; idx1++) {
                    if (cancelCalculation) break;
                    while (isPaused && !cancelCalculation) await new Promise(resolve => setTimeout(resolve, 100));
                    if (cancelCalculation) break;
                    
                    const i = currentChunkIndices[idx1];
                    const marker1 = intervisibilityMarkers[i];
                    const data1 = currentChunkProfiles.get(i);
                    
                    // Check pairs within current chunk (i < j)
                    for (let idx2 = idx1 + 1; idx2 < currentChunkIndices.length; idx2++) {
                        if (cancelCalculation) break;
                        while (isPaused && !cancelCalculation) await new Promise(resolve => setTimeout(resolve, 100));
                        if (cancelCalculation) break;
                        
                        const j = currentChunkIndices[idx2];
                        const marker2 = intervisibilityMarkers[j];
                        const data2 = currentChunkProfiles.get(j);
                        
                        if (!data1 || !data2) continue;
                        checked++;
                        updateIntervisibilityStatus(`Phase 2: Checking pair ${checked}/${total} (${marker1.name} ↔ ${marker2.name})...`);
                        
                        const isVisible = await checkIntervisibility(marker1.latlng, marker2.latlng, data1.observerH, data2.observerH, null, null, data1.profile, data2.profile);
                        if (isVisible) allVisiblePairs.push({ marker1: marker1, marker2: marker2 });
                    ...
                    // Check pairs: current chunk marker vs all markers from previous chunks
                    for (let prevChunkIdx = 0; prevChunkIdx < chunkIdx; prevChunkIdx++) {
                        if (cancelCalculation) break;
                        const prevChunkProfiles = loadedChunkProfiles[prevChunkIdx];
                        const prevChunkIndices = Array.from(prevChunkProfiles.keys()).sort((a, b) => a - b);
                        
                        for (let prevIdx = 0; prevIdx < prevChunkIndices.length; prevIdx++) {
                            if (cancelCalculation) break;
                            while (isPaused && !cancelCalculation) await new Promise(resolve => setTimeout(resolve, 100));
                            if (cancelCalculation) break;
                            
                            const j = prevChunkIndices[prevIdx];
                            const marker2 = intervisibilityMarkers[j];
                            const data2 = prevChunkProfiles.get(j);
                ...
                // Clear current chunk from memory after checking all pairs (keep in loadedChunkProfiles for next chunks)
                // Note: We keep it in loadedChunkProfiles array but clear the Map to free memory
                // Actually, we should keep it for cross-chunk comparisons, so we don't clear it here
                cleanupDemCache();
                if (window.gc) { try { window.gc(); } catch (e) {} }
                await new Promise(resolve => setTimeout(resolve, 50));
            }
            
            // Clear all loaded chunk profiles
            loadedChunkProfiles.forEach(chunk => chunk.clear());
            loadedChunkProfiles.length = 0;
            
            // Add all visible pairs to the main array
            visiblePairs.push(...allVisiblePairs);
            
        } else if (useChunking && !canLoadChunks) {
            // CHUNKED PROCESSING WITH DOWNLOADS FOLDER: Cannot load chunks back - recalculate profiles
            updateIntervisibilityStatus('Phase 2: Downloads folder used - recalculating profiles in memory for pair checking...');
            const markerDataForPairs = new Map();
            const totalChunks = Math.ceil(intervisibilityMarkers.length / CHUNK_SIZE);
            ...
            for (let chunkIdx = 0; chunkIdx < totalChunks; chunkIdx++) {
                ...
                const startIdx = chunkIdx * CHUNK_SIZE;
                const endIdx = Math.min(startIdx + CHUNK_SIZE, intervisibilityMarkers.length);
                updateIntervisibilityStatus(`Phase 2: Chunk ${chunkIdx + 1}/${totalChunks}: Recalculating profiles...`);
                ...
                for (let i = startIdx; i < endIdx; i++) {
                    ...
                    try {
                        const result = await calculateMarkerProfile(marker, map, i + 1, intervisibilityMarkers.length);
                        markerDataForPairs.set(i, { profile: result.profile, observerH: result.observerH });
                        if (result.dem) {
                            if (result.dem.data) result.dem.data = null;
                            result.dem = null;
                        }
                    } catch (error) {
                        if (!cancelCalculation) console.error(`Error recalculating profile: ${marker.name}:`, error);
                        break;
                    }
                    await new Promise(resolve => setTimeout(resolve, 0));
                }
            ...
            // Check pairs using recalculated profiles (same logic as non-chunked below)
            const completedMarkerIndices = Array.from(markerDataForPairs.keys()).sort((a, b) => a - b);
            for (let idx1 = 0; idx1 < completedMarkerIndices.length; idx1++) {
                ...
                const marker1 = intervisibilityMarkers[i];
                const data1 = markerDataForPairs.get(i);
                for (let idx2 = idx1 + 1; idx2 < completedMarkerIndices.length; idx2++) {
                    ...
                    const marker2 = intervisibilityMarkers[j];
                    const data2 = markerDataForPairs.get(j);
                    if (!data1 || !data2) continue;
                    checked++;
                    updateIntervisibilityStatus(`Phase 2: Checking pair ${checked}/${total} (${marker1.name} ↔ ${marker2.name})...`);
                    const isVisible = await checkIntervisibility(marker1.latlng, marker2.latlng, data1.observerH, data2.observerH, null, null, data1.profile, data2.profile);
                    if (isVisible) visiblePairs.push({ marker1: marker1, marker2: marker2 });
                    await new Promise(resolve => setTimeout(resolve, 0));
                    if (checked % 5 === 0) {
                        cleanupDemCache();
                        if (window.gc) { try { window.gc(); } catch (e) {} }
            ...
            markerDataForPairs.clear();
        } else {
            // NON-CHUNKED PAIR CHECKING: Original logic for small site counts
        ...
        // Store visiblePairs globally immediately so report button can be shown even if line-creation throws
        ...
        if (cancelCalculation) {
            ...
            // Show buttons so user can still save/view/generate report from partial results
            $('#btn-save-intervisibility-matrix').show();
            $('#btn-view-intervisibility-statistics').show();
            $('#btn-generate-iv-report').prop('disabled', false);
            var statsEl = document.getElementById('intervisibility-statistics');
            ...
            $('#btn-save-intervisibility-matrix').show();
            // Show View Statistics and enable Generate Report button
            $('#btn-view-intervisibility-statistics').show();
            $('#btn-generate-iv-report').prop('disabled', false);
    ...
    window.saveIntervisibilityMatrix = function() {
        ...
        intervisibilityMarkers.forEach(marker => {
            features.push({
                ...
                properties: {
                    name: marker.name,
                    uniqueId: marker.uniqueId || marker.name || null,
                    elevation: marker.elevation || 0,
            ...
            try {
                ...
                if (markerFeatures.length > 0) {
                    ...
                    markerFeatures.forEach((feature, index) => {
                        ...
                        const uniqueId = props.uniqueId || name || `Site ${index + 1}`;
                ...
                if (geoJson.metadata && loadedVisiblePairs.length > 0 && intervisibilityMarkers.length > 0) {
                    ...
                    $('#btn-view-intervisibility-statistics').hide();
                    $('#btn-generate-iv-report').prop('disabled', true);
                }
    ...
    function clearIntervisibilityData() {
        ...
        window.visiblePairsForStats = [];
        $('#btn-generate-iv-report').prop('disabled', true);
    }
    
    /**
     * Return current IV result for report generation, or null if none.
     */
    window.getIVResultForReport = function() {
        if (!window.visiblePairsForStats || window.visiblePairsForStats.length === 0 || intervisibilityMarkers.length === 0) {
            return null;
        }
        const n = intervisibilityMarkers.length;
        const visiblePairsIndex = [];
        window.visiblePairsForStats.forEach(function(pair) {
            const idx1 = intervisibilityMarkers.findIndex(function(m) { return m.latlng.toString() === pair.marker1.latlng.toString(); });
            const idx2 = intervisibilityMarkers.findIndex(function(m) { return m.latlng.toString() === pair.marker2.latlng.toString(); });
            if (idx1 !== -1 && idx2 !== -1) visiblePairsIndex.push({ i: idx1, j: idx2 });
        });
        const totalPairs = (n * (n - 1)) / 2;
        const visibleCount = visiblePairsIndex.length;
        const percent = totalPairs > 0 ? ((visibleCount / totalPairs) * 100).toFixed(1) : 0;
        return {
            markers: intervisibilityMarkers.map(function(m, i) { return { name: m.name, uniqueId: m.uniqueId, index: i }; }),
            visiblePairsIndex: visiblePairsIndex,
            runAt: new Date().toISOString(),
            summary: { totalPairs: totalPairs, visibleCount: visibleCount, percent: percent },
            totalSites: n
        };
    };
    
    /**
     * Generate IV report HTML, save to localStorage, and open in new window.
     */
    window.generateIVReport = function() {
        const data = window.getIVResultForReport();
        if (!data) {
            if (typeof updateIntervisibilityStatus === 'function') updateIntervisibilityStatus('No IV result available. Create an intervisibility matrix first.');
            return;
        }
        if (typeof window.IVReportUtils === 'undefined') {
            if (typeof updateIntervisibilityStatus === 'function') updateIntervisibilityStatus('IV report utilities not loaded.');
            return;
        }
        const n = data.totalSites;
        const adj = window.IVReportUtils.buildAdjacencyFromVisiblePairs(n, data.visiblePairsIndex);
        const components = window.IVReportUtils.countComponents(adj);
        const diameter = window.IVReportUtils.getDiameter(adj);
        const avgPathLen = window.IVReportUtils.getAveragePathLength(adj);
        const betweenness = window.IVReportUtils.betweennessCentrality(adj);
        const closeness = window.IVReportUtils.closenessCentrality(adj);
        const degrees = {};
        data.visiblePairsIndex.forEach(function(p) {
            degrees[p.i] = (degrees[p.i] || 0) + 1;
            degrees[p.j] = (degrees[p.j] || 0) + 1;
        });
        for (let i = 0; i < n; i++) if (degrees[i] === undefined) degrees[i] = 0;
        
        const runDate = new Date(data.runAt).toLocaleString();
        const rows = data.markers.map(function(m, i) {
            const deg = degrees[m.index];
            const bc = (betweenness[m.index] != null) ? betweenness[m.index].toFixed(4) : '—';
            const cc = (closeness[m.index] != null) ? closeness[m.index].toFixed(4) : '—';
            return '<tr><td>' + (m.index + 1) + '</td><td>' + (m.uniqueId || m.name || 'Site ' + (m.index + 1)) + '</td><td>' + deg + '</td><td>' + bc + '</td><td>' + cc + '</td></tr>';
        }).join('');
        
        const html = '<!DOCTYPE html><html><head><meta charset="utf-8"><title>Intervisibility Report</title><style>body{font-family:Montserrat,sans-serif;margin:20px;max-width:900px;}h1{color:#023020;}h2{margin-top:24px;color:#2487CE;}table{border-collapse:collapse;width:100%;margin-top:8px;}th,td{border:1px solid #ddd;padding:8px;text-align:left;}th{background:#f3f4f6;}.meta{color:#666;font-size:14px;margin-bottom:20px;}.summary{display:flex;flex-wrap:wrap;gap:16px;margin:16px 0;}.summary div{background:#f9f9f9;padding:12px 16px;border-radius:4px;min-width:120px;}</style></head><body>' +
            '<h1>Intervisibility Analysis Report</h1>' +
            '<p class="meta">Generated: ' + runDate + '</p>' +
            '<h2>Summary</h2>' +
            '<div class="summary">' +
            '<div><strong>Sites</strong><br>' + n + '</div>' +
            '<div><strong>Total pairs</strong><br>' + data.summary.totalPairs + '</div>' +
            '<div><strong>Intervisible pairs</strong><br>' + data.summary.visibleCount + '</div>' +
            '<div><strong>Intervisibility ratio</strong><br>' + data.summary.percent + '%</div>' +
            '</div>' +
            '<h2>Global network</h2>' +
            '<div class="summary">' +
            '<div><strong>Components</strong><br>' + components + '</div>' +
            '<div><strong>Diameter</strong><br>' + (diameter != null ? diameter : '—') + '</div>' +
            '<div><strong>Avg path length</strong><br>' + (avgPathLen != null ? avgPathLen.toFixed(2) : '—') + '</div>' +
            '</div>' +
            '<h2>Per-site metrics</h2>' +
            '<table><thead><tr><th>#</th><th>Site</th><th>Degree</th><th>Betweenness</th><th>Closeness</th></tr></thead><tbody>' + rows + '</tbody></table>' +
            '</body></html>';
        
        try {
            const reports = JSON.parse(localStorage.getItem('mace_iv_reports') || '[]');
            reports.push({ id: Date.now(), title: 'IV Report ' + runDate, createdAt: data.runAt, html: html, ivData: { runAt: data.runAt, summary: data.summary, totalSites: n } });
            localStorage.setItem('mace_iv_reports', JSON.stringify(reports));
        } catch (e) { /* ignore */ }
        
        const blob = new Blob([html], { type: 'text/html;charset=utf-8' });
        const url = URL.createObjectURL(blob);
        const w = window.open(url, '_blank', 'noopener');
        if (w) setTimeout(function() { URL.revokeObjectURL(url); }, 1000);
    };
    ...
    $(document).ready(function() {
        ...
        $('#btn-generate-iv-report').click(function() {
            if (typeof window.generateIVReport === 'function') {
                window.generateIVReport();
            }
        });
    });
