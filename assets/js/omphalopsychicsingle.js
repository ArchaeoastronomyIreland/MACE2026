...
   azia = parseFloat(azi) * Math.PI / 180
   ...
   // Suppressed: latitude can be negative (southern hemisphere)
...
   }
   ...
   // Suppressed: error latitude validation
   // if (errorlata < 0 || isNaN(errorlata)) {
   //    alert("ERROR: ERROR LATITUDE should be positive")
   // }
   if (isNaN(errorlata)) {
      alert("ERROR: ERROR LATITUDE must be a valid number")
   }
   if (methoda == 1) {
      kma = Km
      if (kma <= 0 || isNaN(kma)) {
...
   // Store base calcalt before disk compensation (for calculating all three states)
   const calcalt_base = calcalt;
   
   // Calculate all three disk states for astronomical calculations
   // diska values: 1 = Upper Limb, 0 = Center, -1 = Lower Limb
   // Semidiameter compensation: 16 arcminutes = 16/60 degrees
   const semidiameter_rad = 16 / 60 / 180 * Math.PI;
   
   // Calculate altitudes for all three states
   const calcalt_UL = calcalt_base + 1 * semidiameter_rad;  // Upper Limb
   const calcalt_Center = calcalt_base + 0 * semidiameter_rad;  // Center
   const calcalt_LL = calcalt_base + (-1) * semidiameter_rad;  // Lower Limb
   
   // Use checkbox-selected state for View Azimuths and orthodrome calculations
   // This preserves existing behavior for ready reckoner display
   calcalt = calcalt_base + diska * semidiameter_rad;
   
   // Note: calcalta and calcaltb will be recalculated for major/minor lunar with parallax
   // We'll calculate all three states for those separately
...
   // form.test.value = obliquity/Math.PI*180
   ...
   // Helper function to calculate azimuth for a given declination and altitude
   const calcAzimuth = (declRad, altRad) => {
      return 180 / Math.PI * Math.acos((Math.sin(declRad) - Math.sin(lata) * Math.sin(altRad)) / Math.cos(lata) / Math.cos(altRad));
   };
   
   // Calculate zero-horizon azimuths for all three disk states (for astronomical calculations)
   // UL = Upper Limb, Center = Center, LL = Lower Limb
   const calcAllStatesAzimuths = (declRad) => {
      return {
         UL: {
            rise: calcAzimuth(declRad, calcalt_UL),
            set: 360 - calcAzimuth(declRad, calcalt_UL)
         },
         Center: {
            rise: calcAzimuth(declRad, calcalt_Center),
            set: 360 - calcAzimuth(declRad, calcalt_Center)
         },
         LL: {
            rise: calcAzimuth(declRad, calcalt_LL),
            set: 360 - calcAzimuth(declRad, calcalt_LL)
         }
      };
   };
   
   // Calculate solstice azimuths using checkbox-selected state (for View Azimuths and orthodromes)
   azimuth = calcAzimuth(hoek, calcalt);
   ...
   // Calculate and store all three states for astronomical calculations
   const solsticeStates = calcAllStatesAzimuths(hoek);
   const solsticeStatesWin = calcAllStatesAzimuths(-hoek);
   
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.solsticeazisumrise = solsticeazisumrise;
   window.solsticeazisumset = solsticeazisumset;
   window.solsticeaziwinrise = solsticeaziwinrise;
   window.solsticeaziwinset = solsticeaziwinset;
   
   // Set window variables for all three states (for astronomical calculations)
   window.solsticeazisumrise_UL = Math.round(solsticeStates.UL.rise * 100) / 100;
   window.solsticeazisumset_UL = Math.round(solsticeStates.UL.set * 100) / 100;
   window.solsticeazisumrise_Center = Math.round(solsticeStates.Center.rise * 100) / 100;
   window.solsticeazisumset_Center = Math.round(solsticeStates.Center.set * 100) / 100;
   window.solsticeazisumrise_LL = Math.round(solsticeStates.LL.rise * 100) / 100;
   window.solsticeazisumset_LL = Math.round(solsticeStates.LL.set * 100) / 100;
   window.solsticeaziwinrise_UL = Math.round(solsticeStatesWin.UL.rise * 100) / 100;
   window.solsticeaziwinset_UL = Math.round(solsticeStatesWin.UL.set * 100) / 100;
   window.solsticeaziwinrise_Center = Math.round(solsticeStatesWin.Center.rise * 100) / 100;
   window.solsticeaziwinset_Center = Math.round(solsticeStatesWin.Center.set * 100) / 100;
   window.solsticeaziwinrise_LL = Math.round(solsticeStatesWin.LL.rise * 100) / 100;
   window.solsticeaziwinset_LL = Math.round(solsticeStatesWin.LL.set * 100) / 100;
   
   window.status = "Calculations finished"

   // Still write to form fields for View Azimuths display (ready reckoner)
   if (typeof $("#solsticeazisumrise").val === 'function') {
...
   sooud = soac
   ...
   // Calculate cross-quarter azimuths using checkbox-selected state (for View Azimuths and orthodromes)
   azimuthc = calcAzimuth(hoek3, calcalt);
   ...
   // Calculate and store all three states for astronomical calculations
   const crossquarterStates = calcAllStatesAzimuths(hoek3);
   const crossquarterStatesWin = calcAllStatesAzimuths(-hoek3);
   
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.crossquarterazisumrise = crossquarterazisumrise;
   window.crossquarterazisumset = crossquarterazisumset;
   window.crossquarteraziwinrise = crossquarteraziwinrise;
   window.crossquarteraziwinset = crossquarteraziwinset;
   
   // Set window variables for all three states (for astronomical calculations)
   window.crossquarterazisumrise_UL = Math.round(crossquarterStates.UL.rise * 100) / 100;
   window.crossquarterazisumset_UL = Math.round(crossquarterStates.UL.set * 100) / 100;
   window.crossquarterazisumrise_Center = Math.round(crossquarterStates.Center.rise * 100) / 100;
   window.crossquarterazisumset_Center = Math.round(crossquarterStates.Center.set * 100) / 100;
   window.crossquarterazisumrise_LL = Math.round(crossquarterStates.LL.rise * 100) / 100;
   window.crossquarterazisumset_LL = Math.round(crossquarterStates.LL.set * 100) / 100;
   window.crossquarteraziwinrise_UL = Math.round(crossquarterStatesWin.UL.rise * 100) / 100;
   window.crossquarteraziwinset_UL = Math.round(crossquarterStatesWin.UL.set * 100) / 100;
   window.crossquarteraziwinrise_Center = Math.round(crossquarterStatesWin.Center.rise * 100) / 100;
   window.crossquarteraziwinset_Center = Math.round(crossquarterStatesWin.Center.set * 100) / 100;
   window.crossquarteraziwinrise_LL = Math.round(crossquarterStatesWin.LL.rise * 100) / 100;
   window.crossquarteraziwinset_LL = Math.round(crossquarterStatesWin.LL.set * 100) / 100;
   
   window.status = "Calculations finished"

   // Still write to form fields for View Azimuths display (ready reckoner)
   if (typeof $("#crossquarterazisumrise").val === 'function') {
...
   sooud = soab
   ...
   // Calculate equinox azimuths using checkbox-selected state (for View Azimuths and orthodromes)
   azimuthb = calcAzimuth(hoek2, calcalt);
   ...
   // Calculate and store all three states for astronomical calculations
   const equinoxStates = calcAllStatesAzimuths(hoek2);
   const equinoxStatesWin = calcAllStatesAzimuths(-hoek2);
   
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.equinoxazisumrise = equinoxazisumrise;
   window.equinoxazisumset = equinoxazisumset;
   window.equinoxaziwinrise = equinoxaziwinrise;
   window.equinoxaziwinset = equinoxaziwinset;
   
   // Set window variables for all three states (for astronomical calculations)
   window.equinoxazisumrise_UL = Math.round(equinoxStates.UL.rise * 100) / 100;
   window.equinoxazisumset_UL = Math.round(equinoxStates.UL.set * 100) / 100;
   window.equinoxazisumrise_Center = Math.round(equinoxStates.Center.rise * 100) / 100;
   window.equinoxazisumset_Center = Math.round(equinoxStates.Center.set * 100) / 100;
   window.equinoxazisumrise_LL = Math.round(equinoxStates.LL.rise * 100) / 100;
   window.equinoxazisumset_LL = Math.round(equinoxStates.LL.set * 100) / 100;
   window.equinoxaziwinrise_UL = Math.round(equinoxStatesWin.UL.rise * 100) / 100;
   window.equinoxaziwinset_UL = Math.round(equinoxStatesWin.UL.set * 100) / 100;
   window.equinoxaziwinrise_Center = Math.round(equinoxStatesWin.Center.rise * 100) / 100;
   window.equinoxaziwinset_Center = Math.round(equinoxStatesWin.Center.set * 100) / 100;
   window.equinoxaziwinrise_LL = Math.round(equinoxStatesWin.LL.rise * 100) / 100;
   window.equinoxaziwinset_LL = Math.round(equinoxStatesWin.LL.set * 100) / 100;
   window.status = "Calculations finished"

   // Still write to form fields for View Azimuths display (ready reckoner)
   if (typeof $("#equinoxazisumrise").val === 'function') {
...
   }
   ...
   // Calculate all three states for major lunar (with parallax if soad == 1)
   let calcalta_UL, calcalta_Center, calcalta_LL;
   if (soad == 1) {
      ...
      const parallaxAdj = 1.004 / 180 * Math.PI * (Math.cos(calcalt_base) - Math.sin(calcalt_base * 2) * 0.008258773);
      calcalta = calcalt_base + parallaxAdj + diska * semidiameter_rad;  // Checkbox-selected state
      calcalta_UL = calcalt_base + parallaxAdj + 1 * semidiameter_rad;
      calcalta_Center = calcalt_base + parallaxAdj + 0 * semidiameter_rad;
      calcalta_LL = calcalt_base + parallaxAdj + (-1) * semidiameter_rad;
   } else {
      calcalta = calcalt_base + diska * semidiameter_rad;
      calcalta_UL = calcalt_UL;
      calcalta_Center = calcalt_Center;
      calcalta_LL = calcalt_LL;
   }

   sooud = soad

   // Helper function for major lunar with parallax
   const calcMajorLunarAzimuth = (declRad, altRad) => {
      return 180 / Math.PI * Math.acos((Math.sin(declRad) - Math.sin(lata) * Math.sin(altRad)) / Math.cos(lata) / Math.cos(altRad));
   };
   
   // Calculate all three states for major lunar
   const majorStates = {
      UL: {
         rise: calcMajorLunarAzimuth(hoek4, calcalta_UL),
         set: 360 - calcMajorLunarAzimuth(hoek4, calcalta_UL)
      },
      Center: {
         rise: calcMajorLunarAzimuth(hoek4, calcalta_Center),
         set: 360 - calcMajorLunarAzimuth(hoek4, calcalta_Center)
      },
      LL: {
         rise: calcMajorLunarAzimuth(hoek4, calcalta_LL),
         set: 360 - calcMajorLunarAzimuth(hoek4, calcalta_LL)
      }
   };
   const majorStatesWin = {
      UL: {
         rise: calcMajorLunarAzimuth(-hoek4, calcalta_UL),
         set: 360 - calcMajorLunarAzimuth(-hoek4, calcalta_UL)
      },
   ...
   // Store all three states
   window.majorazisumrise_UL = Math.round(majorStates.UL.rise * 100) / 100;
   window.majorazisumset_UL = Math.round(majorStates.UL.set * 100) / 100;
   window.majorazisumrise_Center = Math.round(majorStates.Center.rise * 100) / 100;
   window.majorazisumset_Center = Math.round(majorStates.Center.set * 100) / 100;
   window.majorazisumrise_LL = Math.round(majorStates.LL.rise * 100) / 100;
   window.majorazisumset_LL = Math.round(majorStates.LL.set * 100) / 100;
   window.majoraziwinrise_UL = Math.round(majorStatesWin.UL.rise * 100) / 100;
   window.majoraziwinset_UL = Math.round(majorStatesWin.UL.set * 100) / 100;
   window.majoraziwinrise_Center = Math.round(majorStatesWin.Center.rise * 100) / 100;
   window.majoraziwinset_Center = Math.round(majorStatesWin.Center.set * 100) / 100;
   window.majoraziwinrise_LL = Math.round(majorStatesWin.LL.rise * 100) / 100;
   window.majoraziwinset_LL = Math.round(majorStatesWin.LL.set * 100) / 100;

   // Calculate major lunar azimuths using checkbox-selected state (for View Azimuths and orthodromes)
   azimuthd = calcMajorLunarAzimuth(hoek4, calcalta);
   majorazisumrise = Math.round(azimuthd * 100) / 100
   majorazisumset = Math.round((360 - azimuthd) * 100) / 100
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.majorazisumrise = majorazisumrise;
   window.majorazisumset = majorazisumset;
   azimuthd = calcMajorLunarAzimuth(-hoek4, calcalta);
   majoraziwinrise = Math.round(azimuthd * 100) / 100
   majoraziwinset = Math.round((360 - azimuthd) * 100) / 100
   
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.majoraziwinrise = majoraziwinrise;
   window.majoraziwinset = majoraziwinset;
   
   window.status = "Calculations finished"

   // Still write to form fields for View Azimuths display (ready reckoner)
   if (typeof $("#majorazisumrise").val === 'function') {
...
   }
   ...
   // Calculate all three states for minor lunar (with parallax if soae == 1)
   let calcaltb_UL, calcaltb_Center, calcaltb_LL;
   if (soae == 1) {
      ...
      // CRITICAL: Use the already-calculated calcalta values (which include major lunar parallax if soad == 1)
      // The original code used calcalta directly, so we must use the corresponding calcalta_UL, calcalta_Center, calcalta_LL
      // Calculate minor lunar parallax adjustment for each state using the corresponding calcalta value
      const parallaxAdjMinor_UL = 0.900 / 180 * Math.PI * (Math.cos(calcalta_UL) - Math.sin(calcalta_UL * 2) * 0.008258773);
      const parallaxAdjMinor_Center = 0.900 / 180 * Math.PI * (Math.cos(calcalta_Center) - Math.sin(calcalta_Center * 2) * 0.008258773);
      const parallaxAdjMinor_LL = 0.900 / 180 * Math.PI * (Math.cos(calcalta_LL) - Math.sin(calcalta_LL * 2) * 0.008258773);
      
      // For checkbox-selected state, use calcalta (which already has diska adjustment)
      const parallaxAdjMinor = 0.900 / 180 * Math.PI * (Math.cos(calcalta) - Math.sin(calcalta * 2) * 0.008258773);
      calcaltb = calcalta + parallaxAdjMinor;  // Checkbox-selected state
      calcaltb_UL = calcalta_UL + parallaxAdjMinor_UL;
      calcaltb_Center = calcalta_Center + parallaxAdjMinor_Center;
      calcaltb_LL = calcalta_LL + parallaxAdjMinor_LL;
   } else {
      calcaltb = calcalt_base + diska * semidiameter_rad;
      calcaltb_UL = calcalt_UL;
      calcaltb_Center = calcalt_Center;
      calcaltb_LL = calcalt_LL;
   }
   //alert(soa)
   sooud = soae

   // Helper function for minor lunar with parallax
   const calcMinorLunarAzimuth = (declRad, altRad) => {
      return 180 / Math.PI * Math.acos((Math.sin(declRad) - Math.sin(lata) * Math.sin(altRad)) / Math.cos(lata) / Math.cos(altRad));
   };
   
   // Calculate all three states for minor lunar
   const minorStates = {
      UL: {
         rise: calcMinorLunarAzimuth(hoek5, calcaltb_UL),
         set: 360 - calcMinorLunarAzimuth(hoek5, calcaltb_UL)
      },
      Center: {
         rise: calcMinorLunarAzimuth(hoek5, calcaltb_Center),
         set: 360 - calcMinorLunarAzimuth(hoek5, calcaltb_Center)
      },
      LL: {
         rise: calcMinorLunarAzimuth(hoek5, calcaltb_LL),
         set: 360 - calcMinorLunarAzimuth(hoek5, calcaltb_LL)
      }
   };
   const minorStatesWin = {
      UL: {
         rise: calcMinorLunarAzimuth(-hoek5, calcaltb_UL),
         set: 360 - calcMinorLunarAzimuth(-hoek5, calcaltb_UL)
      },
   ...
   // Store all three states
   window.minorazisumrise_UL = Math.round(minorStates.UL.rise * 100) / 100;
   window.minorazisumset_UL = Math.round(minorStates.UL.set * 100) / 100;
   window.minorazisumrise_Center = Math.round(minorStates.Center.rise * 100) / 100;
   window.minorazisumset_Center = Math.round(minorStates.Center.set * 100) / 100;
   window.minorazisumrise_LL = Math.round(minorStates.LL.rise * 100) / 100;
   window.minorazisumset_LL = Math.round(minorStates.LL.set * 100) / 100;
   window.minoraziwinrise_UL = Math.round(minorStatesWin.UL.rise * 100) / 100;
   window.minoraziwinset_UL = Math.round(minorStatesWin.UL.set * 100) / 100;
   window.minoraziwinrise_Center = Math.round(minorStatesWin.Center.rise * 100) / 100;
   window.minoraziwinset_Center = Math.round(minorStatesWin.Center.set * 100) / 100;
   window.minoraziwinrise_LL = Math.round(minorStatesWin.LL.rise * 100) / 100;
   window.minoraziwinset_LL = Math.round(minorStatesWin.LL.set * 100) / 100;

   // Calculate minor lunar azimuths using checkbox-selected state (for View Azimuths and orthodromes)
   azimuthe = calcMinorLunarAzimuth(hoek5, calcaltb);
   ...
   // Set window variables for checkbox-selected state (for View Azimuths and orthodromes)
   window.minorazisumrise = minorazisumrise;
   window.minorazisumset = minorazisumset;
   window.minoraziwinrise = minoraziwinrise;
   window.minoraziwinset = minoraziwinset;
   
   window.status = "Calculations finished"

   // Still write to form fields for View Azimuths display (ready reckoner)
   if (typeof $("#minorazisumrise").val === 'function') {
...
   $('#minor4long').html(minor4long);
    ...
    // =================================================================
    // NEW: Expose Geometric Declination Values for hwtip.js
    // These are pure geometric declination values (no refraction, parallax, semidiameter corrections)
    // They are the same as the regular declination values, but explicitly marked as geometric
    // for clarity and consistency with geometric azimuths
    // =================================================================
    window.declinationSummerSolstice_geo = obliquity * 180 / Math.PI;
    window.declinationWinterSolstice_geo = -obliquity * 180 / Math.PI;
    window.declinationEquinox_geo = hoek2 * 180 / Math.PI; // hoek2 is 0 if equinox is active
    window.declinationCrossQuarterNorth_geo = hoek3 * 180 / Math.PI; // hoek3 is 0.69 * obliquity if active
    window.declinationCrossQuarterSouth_geo = -hoek3 * 180 / Math.PI;
    window.declinationMajorLunarNorth_geo = hoek4 * 180 / Math.PI; // hoek4 includes lunar perturbation
    window.declinationMajorLunarSouth_geo = -hoek4 * 180 / Math.PI;
    window.declinationMinorLunarNorth_geo = hoek5 * 180 / Math.PI; // hoek5 includes lunar perturbation
    window.declinationMinorLunarSouth_geo = -hoek5 * 180 / Math.PI;
    
    // Output geometric declination values for verification
    console.log("--- Geometric Declination Values (for hwtip.js) ---");
    console.log("Summer Solstice (geo):", window.declinationSummerSolstice_geo.toFixed(4));
    console.log("Winter Solstice (geo):", window.declinationWinterSolstice_geo.toFixed(4));
    console.log("Equinox (geo):", window.declinationEquinox_geo.toFixed(4));
    console.log("North Cross-Quarter (geo):", window.declinationCrossQuarterNorth_geo.toFixed(4));
    console.log("South Cross-Quarter (geo):", window.declinationCrossQuarterSouth_geo.toFixed(4));
    console.log("North Major Lunar (geo):", window.declinationMajorLunarNorth_geo.toFixed(4));
    console.log("South Major Lunar (geo):", window.declinationMajorLunarSouth_geo.toFixed(4));
    console.log("North Minor Lunar (geo):", window.declinationMinorLunarNorth_geo.toFixed(4));
    console.log("South Minor Lunar (geo):", window.declinationMinorLunarSouth_geo.toFixed(4));
    console.log("--- End Geometric Declination Values ---");
    // =================================================================

    // =================================================================
    // NEW: Calculate Geometric Zero-Horizon Azimuths for hwtip.js
    // These are calculated at geometric horizon (0°) with NO refraction,
    // NO parallax, and NO semidiameter corrections - pure spherical trigonometry
    // =================================================================
    function calculateGeometricZeroHorizonAzimuths() {
        // Get observer latitude (must be in radians for calculations)
        const observerLatRad = lata; // lata is already in radians from compute()
        
        // Geometric altitude = 0° (true horizon, no corrections)
        const geometricAltitude = 0;
        const geometricAltitudeRad = 0;
        
        // Helper function to calculate azimuth at geometric horizon (altitude = 0°)
        // Correct formula: First find hour angle when altitude = 0, then calculate azimuth
        // At horizon: sin(h) = 0, so: sin(dec)sin(lat) + cos(dec)cos(lat)cos(HA) = 0
        // Therefore: cos(HA) = -tan(dec)tan(lat)
        // Then: azimuth from hour angle using spherical trigonometry
        const calcGeometricAz = (declinationRad) => {
            // Calculate hour angle when body is at geometric horizon (altitude = 0)
            const cosHA = -Math.tan(observerLatRad) * Math.tan(declinationRad);
            if (Math.abs(cosHA) > 1) {
                // Body never rises/sets at this latitude
                return null;
            }
            const haRad = Math.acos(cosHA);
            
            // Calculate azimuth from hour angle and declination
            // Using the correct spherical trigonometry formula
            const sinAz = Math.sin(haRad) * Math.cos(declinationRad) / Math.cos(observerLatRad);
            const cosAz = (Math.sin(declinationRad) - Math.sin(observerLatRad) * Math.cos(haRad)) / (Math.cos(observerLatRad) * Math.sin(haRad));
            let azimuthRad = Math.atan2(sinAz, cosAz);
            azimuthRad = (azimuthRad + 2 * Math.PI) % (2 * Math.PI); // Normalize to 0-2π
            return azimuthRad * 180 / Math.PI; // Convert to degrees
        };
        
        // Calculate geometric zero-horizon azimuths for all events
        // Pattern matches omphalopsychicsingle.js: use hoek for summer, -hoek for winter
        // Solstices
        let azimuth_geo = calcGeometricAz(hoek);
        window.solsticeazisumrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.solsticeazisumset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        azimuth_geo = calcGeometricAz(-hoek);
        window.solsticeaziwinrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.solsticeaziwinset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        
        // Equinoxes (hoek2 = 0 if equinox is active, otherwise = eclipa)
        const equinoxDecl = hoek2; // Already set in compute()
        azimuth_geo = calcGeometricAz(equinoxDecl);
        window.equinoxazisumrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.equinoxazisumset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        azimuth_geo = calcGeometricAz(-equinoxDecl);
        window.equinoxaziwinrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.equinoxaziwinset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        
        // Cross-Quarters (hoek3 = 0.69 * eclipa if cross-quarter is active)
        const crossQuarterDecl = hoek3; // Already set in compute()
        azimuth_geo = calcGeometricAz(crossQuarterDecl);
        window.crossquarterazisumrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.crossquarterazisumset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        azimuth_geo = calcGeometricAz(-crossQuarterDecl);
        window.crossquarteraziwinrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.crossquarteraziwinset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        
        // Major Lunar (hoek4 includes lunar perturbation)
        const majorLunarDecl = hoek4; // Already set in compute()
        azimuth_geo = calcGeometricAz(majorLunarDecl);
        window.majorazisumrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.majorazisumset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        azimuth_geo = calcGeometricAz(-majorLunarDecl);
        window.majoraziwinrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.majoraziwinset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        
        // Minor Lunar (hoek5 includes lunar perturbation)
        const minorLunarDecl = hoek5; // Already set in compute()
        azimuth_geo = calcGeometricAz(minorLunarDecl);
        window.minorazisumrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.minorazisumset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        azimuth_geo = calcGeometricAz(-minorLunarDecl);
        window.minoraziwinrise_geo = Math.round(azimuth_geo * 100) / 100;
        window.minoraziwinset_geo = Math.round((360 - azimuth_geo) * 100) / 100;
        
        // Output for verification
        console.log("--- Geometric Zero-Horizon Azimuths (for hwtip.js) ---");
        console.log("SSR (geo):", window.solsticeazisumrise_geo.toFixed(4));
        console.log("SSS (geo):", window.solsticeazisumset_geo.toFixed(4));
        console.log("WSR (geo):", window.solsticeaziwinrise_geo.toFixed(4));
        console.log("WSS (geo):", window.solsticeaziwinset_geo.toFixed(4));
        console.log("ER (geo):", window.equinoxazisumrise_geo.toFixed(4));
        console.log("ES (geo):", window.equinoxazisumset_geo.toFixed(4));
        console.log("--- End Geometric Zero-Horizon Azimuths ---");
    }
    
    // Call the function to calculate geometric azimuths
    calculateGeometricZeroHorizonAzimuths();
    // =================================================================
